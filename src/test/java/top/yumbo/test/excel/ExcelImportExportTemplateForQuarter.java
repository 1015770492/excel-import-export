...
public class ExcelImportExportTemplateForQuarter {
     ...
     * 年份
     */
    @ExcelCellBindAnnotation(title = "时间", importPattern = "([0-9]{4})年", exception = "年份格式不正确", exportFormat = "%年", exportPriority = 1)
    private Integer year;

    /**
     * 季度，填写1到4的数字
     */
    @ExcelCellBindAnnotation(title = "时间", importPattern = "([1-4]{1})季", exception = "季度只能是1，2，3，4", exportFormat = "第%s季", exportPriority = 2)
    private Integer quarter;

    /**
     * 地区代码，存储最末一级的地区代码就可以
     */
    @ExcelCellBindAnnotation(title = "地区", width = 2, exception = "地区不存在", exportSplit = ",")
     ...
     * 合计违约规模
     */
    @ExcelCellBindAnnotation(title = "合计违约规模", exception = ExceptionMsg.INCORRECT_FORMAT_EXCEPTION, size = BigDecimalUtils.ONE_HUNDRED_MILLION_STRING)
